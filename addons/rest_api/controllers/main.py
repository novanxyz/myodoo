...
def format_error(self,message, accept_type='html'):
    if 'json' in accept_type:
        return request.make_reponse({"error":{"message":message}})
    return  (f"<html><body><h2>{message}</h2></body></html>")
...
class RestApi(http.Controller):
    ...
    def auth_api_key(self, api_key):
        ...
        if api_key is not None and user_id:
            ...
            response = format_error('Invalid API Key ')
        else:
            response = format_error("No API Key Provided ")
    ...
    def generate_response(self, method, model, rec_id):
        ...
        if not fields and method != 'DELETE':
            return format_error("No fields selected for the model")
        if not option:
            return format_error("No Record Created for the model")
        ...
        try:
            if method == 'GET':
                ...
                if not option.is_get:
                    return format_error("Method Not Allowed")
            ...
            return format_error("Invalid JSON Data")
        ...
        if method == 'PUT':
            if not option.is_put:
                ...
                if rec_id == 0:
                    return format_error("No ID Provided")
                    ...
                    if not resource.exists():
                        ...
                        try:
                            ...
                            new_data = json.dumps({'Updated resource': partner_records, })
            ...
            if not option.is_delete:
                ...
                if rec_id == 0:
                    ...
                    if not resource.exists():
                        ...
                        remove = json.dumps({"Resource deleted": records, })

                        resource.unlink()
                        return request.make_response(data=remove)

    @http.route(['/api','/api/<string:model>','/api/<string:model>/<int:id>'],
    ...
    @http.route(['/api/auth'], type="http", auth="none", csrf=False,
                methods=['GET'])
